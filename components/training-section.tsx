...
export function TrainingSection() {
  return (
    <section id="training" className="py-24 bg-card/50 scroll-mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {modules.map((module) => {
            ...
            const card = (
              <Card
                className={`overflow-hidden group transition-all hover:border-accent/50 ${
                  module.locked ? "opacity-75 cursor-default" : "cursor-pointer"
                }`}
              >
                {/* Thumbnail */}
            ...
            return (
              <Link key={module.id} href="/training" className="block">
                {card}
              </Link>
            )
          })}
        ...
        <Card className="mt-12 p-8 bg-secondary/50 border-dashed">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
            ...
            <Button className="bg-accent text-accent-foreground hover:bg-accent/90" asChild>
              <Link href="/training">Upload Content</Link>
            </Button>
          </div>
        </Card>
      </div>

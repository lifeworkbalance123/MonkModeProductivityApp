...
export default function ProgramOnboardingWizard({
  ...
  /** When true (ENABLE_FREE_TRIAL / NEXT_PUBLIC_ENABLE_FREE_TRIAL), new users skip Stripe and start a 3-day trial via API. */
  const freeTrialEnabled = useMemo(() => isProgramFreeTrialEnabled(), [])
  /**
   * Include checkout unless admin skip (`skipPayment`) or free trial mode is on.
   * Trial users never open a Stripe session here.
   */
  const steps = useMemo(() => {
    const hidePayment = skipPayment || freeTrialEnabled
    return hidePayment
      ? ONBOARDING_FLOW_STEPS.filter((s) => s !== 'payment')
      : [...ONBOARDING_FLOW_STEPS]
  }, [skipPayment, freeTrialEnabled])
  ...
  async function handleRecheckAuth() {
    ...
    if (session?.user && selected) {
      ...
      if (freeTrialEnabled) {
        await completeTrialFlow(selected)
        return
      }
  ...
  const completeAdminBypassFlow = useCallback(async (program: SelectedProgram) => {
  ...
  const completeTrialFlow = useCallback(async (program: SelectedProgram) => {
    if (adminBypassNavigatingRef.current) return
    adminBypassNavigatingRef.current = true
    setTrialError(null)

    const intake = { ...formData, selected_program: program }
    const saved = await completeOnboarding(intake, { startProgramTrial: true })
    if (!saved.ok) {
      adminBypassNavigatingRef.current = false
      setTrialError(saved.error ?? 'Could not start your program trial.')
      return
    }
  ...
  useEffect(() => {
    ...
    if (freeTrialEnabled) {
      void completeTrialFlow(selected)
      return
    }
    setCurrentStepIndex(phaseIndexOf('payment'))
  }, [
  ...
  /** Linear back within the flow (questions ↔ auth ↔ payment). */
  function handleBack() {
  ...
  /**
   * Advances after program selection / intro / questions / auth — trial (flag on) or payment step.
   */

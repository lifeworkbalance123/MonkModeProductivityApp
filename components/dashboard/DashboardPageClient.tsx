...
import { GuidedProgramTrialBanner } from '@/components/TrialBanner'
...
export function DashboardPageClient({
  ...
  const { activeProgram, programAccess, loading: programStatusLoading } = useProgramStatus()
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {programAccess &&
      programAccess.canAccess === false &&
      programAccess.reason === 'trial_expired' ? (
        <div className="mx-auto max-w-7xl px-4 pb-2 pt-4 md:pt-2">
          <div className="rounded-lg border border-primary/40 bg-primary/10 px-4 py-3 text-sm text-foreground">
            ...
            <Link href="/join?trial=expired" className="font-medium text-primary underline">
              Purchase to restore full program access
            </Link>
            .
          </div>
        </div>
      ...
      programAccess?.canAccess === true &&
      (programAccess.paymentStatus ?? '').toLowerCase() === 'trial' &&
      programAccess.trialEnd ? (
        <div className="mx-auto max-w-7xl px-4 pb-2 pt-4 md:pt-2">
          <GuidedProgramTrialBanner trialEnd={programAccess.trialEnd} />
        </div>
      ) : null}
      {!ready ? (

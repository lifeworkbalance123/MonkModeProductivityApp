...
export async function POST(request: Request) {
  try {
    ...
    let startProgramTrial = false
    try {
      const raw = (await request.json()) as ProgramIntakePayload & {
        skipPayment?: boolean
        startProgramTrial?: boolean
      }
      body = raw
      skipPayment = raw.skipPayment === true
      startProgramTrial = raw.startProgramTrial === true
    } catch {
      return json({ error: 'Invalid JSON' }, 400)
    }

    if (startProgramTrial && skipPayment) {
      return json({ error: 'Cannot combine trial start with skipPayment' }, 400)
    }
    ...
    if (startProgramTrial) {
      if (!isProgramFreeTrialEnabled()) {
        return json(
          {
            error:
              'Program trial is disabled. Set ENABLE_FREE_TRIAL=true or NEXT_PUBLIC_ENABLE_FREE_TRIAL=true.',
          },
          403,
        )
      }
      ...
      if (row && (row.payment_status === 'paid' || isProgramTrialAccessValid(row))) {
      ...
      if (
        ...
        return json(
          {
            error:
              'Your program trial ended. Purchase access on the Join page to continue.',
          },
          400,
        )
      }
    ...
    return json(
      {
        ...
        trialStarted,
        trialDays: trialStarted ? DEFAULT_PROGRAM_TRIAL_DAYS : undefined,
      },
      200,
    )
  } catch (e) {

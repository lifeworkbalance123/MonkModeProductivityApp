...
export async function GET(req: Request) {
  try {
    ...
    const { data: programRow } = await supabase
      .from('user_programs')
      .select(
        'id, program_type, status, program_day, accountability_preference, payment_status, trial_end',
      )
      .eq('user_id', user.id)
      .eq('status', 'active')
      .limit(1)
    ...
    const allowsProgram = programRowAllowsAccess(
      programRow
        ? {
            id: programRow.id,
            payment_status: programRow.payment_status,
            trial_end: programRow.trial_end,
          }
        : null,
    )

    const activeProgram = programRow && allowsProgram ? programRow : null
    ...
    return NextResponse.json({
      activeProgram: activeProgram
        ? {
            ...
            currentDay: activeProgram.program_day || 1,
            accountabilityPreference: activeProgram.accountability_preference ?? null,
            totalDays:
      ...
      programAccess:
        programRow && !allowsProgram
          ? {
              canAccess: false,
              programId: programRow.id,
              paymentStatus: programRow.payment_status,
              trialEnd: programRow.trial_end,
              reason:
                (programRow.payment_status ?? '').toLowerCase() === 'trial'
                  ? 'trial_expired'
                  : 'access_denied',
            }
          : {
              canAccess: allowsProgram,
              programId: programRow?.id ?? null,
              paymentStatus: programRow?.payment_status ?? null,
              trialEnd: programRow?.trial_end ?? null,
              reason: null,
            },
    })
  } catch (error) {

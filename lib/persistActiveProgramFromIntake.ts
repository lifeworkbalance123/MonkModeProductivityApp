...
import { computeProgramTrialEndCalendarDaysFromNow } from '@/lib/programTrial'

export type PersistProgramOptions = {
  /** Start a time-limited trial row on `user_programs`; omit for paid/admin enrollment. */
  programTrialDays?: number
}
...
export async function persistActiveProgramFromIntake(
  ...
  const trialDays =
    typeof opts?.programTrialDays === 'number' && opts.programTrialDays > 0
      ? opts.programTrialDays
      : null

  const trial_end = trialDays
    ? computeProgramTrialEndCalendarDaysFromNow(trialDays)
    : null
  const payment_status = trialDays ? 'trial' : 'paid'
  ...
  if (
    !trialDays &&
    (pt === 'sprint_standard' || pt === 'sprint_monk' || pt === 'transform')
  ) {
